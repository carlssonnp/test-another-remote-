...
Ans: The main difference betewwen parametric and non-parametric methods is that the former generates parameters which giv extra information explaining the linear relationship between the dep_var and each individual independent variables (<- the advantage of parametric methods). One of the disadvantages of parametric mothods is that parametric mothods can not handle the situation when the dep_var is not linearly separable (i.e., dep_var and independent variables as low correlation coefficients). In addition, normally parametric mothods can't handle outliers or missing values in the sample. 
