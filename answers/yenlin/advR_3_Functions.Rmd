...
`is.function` tells if an object is a function. `is.primitive` tells if a function is a primitive function.
...
The above performs a combine function (first c) to create a vector with an item named c (second c) with value 10 (third c).
...
The function goes down to the lowest level and find x ^ 2, which results in f(x) <- 100. Afterwards, R goes to the command f(x) + 1 and get 100 + 1 = 101, followed by multiplying by two to get 202.
